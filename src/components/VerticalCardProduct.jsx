...
const VerticalCardProduct = ({ category, heading }) => {
    ...
    return (
        <div className='container mx-auto p-4 my-2 relative'>
            ...
            <div className='flex items-center gap-4 md:gap-6 overflow-x-scroll scrollbar-none' ref={scrolElements}>
...

                {
                    loading ? (
                        loadingList.map((product, index) => {
                            return (
                                <div key={index} className='w-full min-w-[200px] md:min-w-[320px] max-w-[200px] md:max-w-[320px] bg-sky-100 rounded-sm shadow'>
                                    <div className='bg-slate-300 h-48 p-4 min-w-[200px] md:min-w-[145px] flex justify-center items-center animate-pulse'>
                                    </div>
                        ...
                        data.map((product, index) => {
                            return (
                                <Link to={"product/" + product?._id} key={product?._id || index} className='w-full min-w-[200px] md:min-w-[320px] max-w-[200px] md:max-w-[320px] bg-sky-100 rounded-sm shadow'>
